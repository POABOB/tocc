...
const travel_histroy = ['無', '曾出國', '曾至國內旅遊', '曾至屏東枋山'];
const occupation = [
  '無',
  '醫院工作者',
  '交通運輸業',
...
function getList() {
  ...
  const m = $('#date').val();
  axios
    .get(`/tocc/api/list?cusid=${customer_data.cusid}&month=${m}`)
    ...
    data.forEach((item) => {
      $('.list tbody tr:last').after(
        `<tr>
                    ...
                    <td>${identity[item.identity]}(${item.identity_other})</td>
                    <td>${item.residence}</td>
                    <td>${travel_histroy[item.travel_histroy]}(${
          item.travel_country || item.travel_destination
        })</td>
                    <td>${occupation[item.occupation]}(${
          item.occupation_other
        })</td>
                    <td>${getContactHistory(
                      contact_history,
                      item,
...
function getDate() {
  ...
  var nm = padLeft(today.getMonth() + 1);
  var nd = padLeft(today.getDate());
  var ndate = ny + '-' + nm + '-' + nd;
  $('#date').val(date);
  $('#date').attr({ min: ndate, max: date });
}
