...
async function formValidate() {
  const error = {
    ...
    cellphone: {
      value:
        $('#cellphone').val() == '' ||
        $('#cellphone').val().toString().length !== 10
          ? true
          : false,
      name: 'cellphone',
    },
  ...
  const isError = Object.values(error).some((item) => {
    if (item.value === true) {
      ...
      $('html, body').animate(
        { scrollTop: $(`#${item.name}`).offset().top },
        'fast'
      );
      $(`#${item.name}`).focus();
      return true;
...
async function send() {
  ...
  if (!isError) {
    ...
    const identity_other = $('#identity_other').val();
    ...
    const contact_history = $('input[name="q3"]:checked')
      .map(function () {
        return $(this).val();
      })
      .get();
    ...
    const cluster = $('input[name="q4"]:checked')
      .map(function () {
        return $(this).val();
      })
      .get();
    const cluster_multi = $('input[name="q4-1"]:checked').val();
    const cluster_date = formatDate(
      $('#q4-1-2_month').val(),
      $('#q4-1-2_date').val()
    );
    let data = {
